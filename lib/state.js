...
function StateMachine() {
  ...
  this.auditInterval = setInterval(function() {
    if (self._state >= 5) {
      self.send({
        to: 'audit',
        sent: self.auditSent + 1, // include the current message in the sent count
        received: self.auditReceived
      });
    }
  }, 25000);
}
...
StateMachine.prototype._sendPacket = function(data){
  this.auditSent++;
  log.info('[C '+ (this.socketConfig && this.socketConfig.userId ? this.socketConfig.userId : '') +'] Out', data);
